...
async function loginfetch(username, password) {
    let response = await fetch(`http://localhost:90/api/authentication?username=${username}&password=${password}`, {method: 'GET'});
...
async function calc(A, B) {            
    let response = await fetch(`http://localhost:90/api/calculation?a=${A}&b=${B}`, {method: 'GET'});
    let content = await response.text();
